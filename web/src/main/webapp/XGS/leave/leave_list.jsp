...
<div class="action  divaction">
	...
	<!--增加报销单 区域 开始-->
		<table width="90%" border="0" cellspacing="0" cellpadding="0" class="list items">
	      ...
	      <s:iterator value="#session.leaveList" id="leave" begin="0" status="s">
	      <tr>
	        <td><a href="leave_getLeaveById.action?leave.id=<s:property value="#session.leave.id"/>"><s:property value="#leave.id"/></a></td>
	        <td><s:property value="#leave.employee_sn.name"/>请假<s:property value="#leave.leaveday"/>天</td>
	        <td><s:date name="#leave.createtime" format="yyyy-MM-dd HH:mm"/></td>
	        <td><s:date name="#leave.modifytime" format="yyyy-MM-dd HH:mm"/></td>
	        <td><s:property value="#leave.approve_opinion"/></td>
	        <td><s:property value="#leave.status"/></td>
	        <td>
	       	 <a href="leave_getLeaveById?leave.id=<s:property value="#leave.id"/>"><img src="${images}/search.gif" width="16" height="15" /></a>
	       	  <s:if test="#leave.next_deal_sn.name == #session.employee.name">
		        <s:if test="#leave.status == '待审批'">
	       	 		<a href="leave_getLeaveById2?leave.id=<s:property value="#leave.id"/>">
